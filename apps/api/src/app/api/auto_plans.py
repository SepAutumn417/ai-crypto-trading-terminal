...
def _persist_snapshot(snapshot) -> MarketStructureSnapshotModel:
    """将 StructureSnapshot（内存对象）转为 ORM 模型，供 db.add 后落库。

    candidate_plans.structure_snapshot_id 有外键约束指向
    market_structure_snapshots.id，因此 scan_candidates 必须先将
    snapshot 持久化，否则会触发 ForeignKeyViolationError。
    """
    return MarketStructureSnapshotModel(
        id=snapshot.id,
...
async def scan_candidates(
    ...
    for c in candidates:
        ...
        # 持久化结构快照（仅一次，candidate_plans.structure_snapshot_id 外键约束要求）
        # 必须 flush 确保 snapshot 先于 candidate_plans 插入（无 relationship，UoW 无法自动排序）
        if not snapshot_persisted:
            db.add(_persist_snapshot(snapshot))
            await db.flush()
...
async def list_candidates(
    ...
    symbol: str | None = Query(default=None, description="按标的过滤"),
    grade: str | None = Query(default=None, description="按评级过滤"),
